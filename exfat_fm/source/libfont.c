...
struct t_font_description
{
	...
	short fw[256]; // Character width
	short fy[256]; // Character y correction
};
...
float GetFontHeight()
{
	return font_datas.sy;
}

float GetFontWidth()
{
	return font_datas.sx;
}
...
float DrawString(float x, float y, char *str)
{
	...
	if(font_datas.current_font == 8) {
		...
		if(font_datas.bkcolor) {
			...
			tiny3d_VertexPos(x + (float) len, y + (float) Y_ttf , font_datas.Z);

			tiny3d_VertexPos(x , y + (float) Y_ttf , font_datas.Z);
