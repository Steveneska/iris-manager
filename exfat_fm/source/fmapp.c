...
// Font 0: 224 characters from 32 to 255, 16x32 pix 2 bit depth
#include "font.h"

// Font 1: 224 characters from 32 to 255, 16x32 pix 2 bit depth
#include "font_b.h"

// Font 2: 255 characters from 0 to 254, 8x8 pix 1 bit depth
...
void LoadTexture()
{
	...
	u32 * texture_pointer; // Used to assign texture space without changes texture_mem
...
static void action_delete(void)
{
	...
	if (ps->path)
	{
		...
		// Remove files - show dialog box to confirm selected items
		else if(ps->sels)
		{
...
// 3rd
// Auto mount logic, every X update: X = 60fps * 4sec = 240 frames
...
int fmapp_update(int dat)
{
	...
	else if (NPad (BUTTON_L3))
	{
		...
		{
			fm_status_set ("", 1, 0xffeeeeFF);
			// Check if we're allowed to create directory
...
int fmapp_render(int dat)
{
	...
	#endif
	// Change to 2D context (virtual size of the screen is 848x512 pixels)
	fm_panel_draw (&lp);
	fm_panel_draw (&rp);
...
s32 fmapp_run()
{
	...
	// Everything is setup. Now for the main loop
	while(1)
	{
		// 3
		if (fmapp_update (0) == -1)
