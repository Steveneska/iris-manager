...
// np is normalized path offset
// from path fat0:/abc/ means np is 3 such that norm path is path + 3 -> 0:/abc/
...
// Probe for supported FS on various devices
...
int rootfs_probe ()
{
	...
	for (k = 0; k < sizeof (rootfs) / sizeof (struct fs_root); k++)
	{
		if (rootfs[k].fs && rootfs[k].devid > 0)
		{
			...
			// FAT
			if (fs_fat_k && rootfs[k].fs_type == FS_TFAT)
			{
...
static int fat_scan_path (struct fm_panel *p)
{
	...
	if (res != FR_OK)
		return res;
	// Strip the fat prefix on path from fat0:/ to 0:/
