...
#define DEVICE_TYPE_DVD		0x10 // DVD-ROM, DVD+-R, DVD+-RW etc, they are different by book type field in some SCSI command
#define DEVICE_TYPE_CD		0x08 // CD-ROM, CD-DA, CD-R, CD-RW, etc, they are different somehow with SCSI commands
...
enum DiscType
{
	...
	DISC_TYPE_DVD,		// Any kind of DVD (ROM, +-R, +-RW, etc) that is not a PS game. To distinguish between types, use cobra_get_disc_phys_info to check book type
	DISC_TYPE_CD,		// Any kind of CD (ROM, CDDA, -R, -RW) that is not a PS game. No idea how to distinguish the type :)
	DISC_TYPE_UNKNOWN	// You shouldn't see this value. There is a possibility of this value to be reported on SCDA, since I don't have any, I haven't been able to verify
};

enum PS2EmuType
{
	PS2_EMU_HW, // The so called "full" backwards compatible models, CECHA and CECHB
	PS2_EMU_GX, // The so called partial backwards compatible models, CECHC and CECHE
	PS2_EMU_SW, // The non backwards compatible models
};
...
// Cobra.c - Minimal Cobra library support for Game Launcher
...
typedef struct
{
	...
	uint8_t bd_video_region;	// One of BD Region, or 0 for default
	uint8_t dvd_video_region;	// One of DVD Region or 0 for default
	uint8_t ps2softemu;			// Non-zero to show PS2 discs in non backwards compatible consoles
	uint32_t spoof_version;		// Version in BCD, example: 0x0400 -> 4.00, 0x0420 -> 4.20. Max value: 0x0999
