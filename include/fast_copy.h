...
static int copy_is_split = 0; // Return 1 if files are split
...
{
	s64 readed; // Global bytes read
	s64 writed; // Global bytes written
	...
	int bigfile_mode;
	int pos_path; // Filename position used in big files
...
static int fast_copy_add(char *pathr, char *pathw, char *file)
{
	...
	if(strl > 6)
	{
		...
		{
			if(p[4] != '0' ||  p[5] != '0')  {return 0;} // Ignore these files
			fast_files[fast_num_files].bigfile_mode = 2; // Join split files
		}
	...
	if(strl > 7)
	{
		...
		if(p[0] == '.' && p[2] == '.' && p[3] == 'p')
		{
			if(p[4] == 'a' ||  p[5] == 'r' ||  p[6] == 't')  {return 0;} // Ignore these files
		}
	}
	...
	if(strl > 8)
	{
		...
		if(p[0] == '.' && p[3] == '.' && p[4] == 'p')
		{
			if(p[5] == 'a' ||  p[6] == 'r' ||  p[7] == 't')  {return 0;} // Ignore these files
		}
	}
	...
	{
		fast_files[fast_num_files].bigfile_mode = 3; // Join split files
	}
		...
		fast_writing || files_opened)
	{
		...
		if(fast_writing > 1)
		{
			...
			if(fast_files[current_fast_file_w].writed >= fast_files[current_fast_file_w].len)
			{
				...
				fast_files[current_fast_file_w].fl = 4; // End of process
		...
		else
		{
			...
			if(abs(time_left - tleft) >= 10) // More than 10 secs difference, update time
				time_left = tleft;
