...
extern unsigned new_pad;	// New pad buttons pressed (can only see once, when it change from 0 to 1)
extern unsigned old_pad;	// Old pad buttons pressed (can only change when you release the button)

extern int pad_alive;		// If 1, pad data is valid

extern int rumble1_on;		// Used for rumble
extern int rumble2_on;

unsigned ps3pad_read();		// Read the first connected pad
