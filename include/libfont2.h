...
/*
 * NOTE:
 * LibFont is thinking to work with Tiny3D 2D mode.
 * You need to call tiny3d_Project2D() before to work with draw functions
 */
 ...
 * two fonts: one that starts at chr 32 and finish at chr 255 and another that starts at 0 and finish at 254. Fonts can
 * have one depth of 1, 2, 4 or 8 bit (used as pixel intensity) to create smooth fonts and you can select the byte order
 * (some fonts can use bit 7 as first pixel or bit 0) .
 *
 ...
 * w and h must be 8, 16, 32, etc...
 *
 * The callback is used to create the font externally (it don't need to use Free Type library directly)
 *
 * It receive one RSX texture pointer and return the texture pointer increased and aligned to 16 bytes think to use this
 * pointer to build the next RSX texture.
 */
...
// Used with FONT_DOUBLE_TEXTUREMOD for double texture method: second texture coordinates is calculated
// using (virtual_screen % mod)/mod relation
...
// Font are resizable: the minimum is 8x8 but you can use as you want. Remember the font quality depends on
// the original size
...
// Change the screen width/height limits (usually 848 x 512.0. It is used only for center function and one of
// multi texture modes)
