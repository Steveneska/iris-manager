...
void SaveFavourites(char * path, int mode)
{
	...
	SaveFile(path, (void *) &favourites, sizeof(favourites)); // Use favorites_gamebase?
}
...
void UpdateFavourites(t_directories *list, int nlist)
{
	...
	for(m = 0; m < num_box; m++)
	{
		...
		for(n = 0; n < nlist; n++)
		{
			...
			{
				if((favourites.list[m].index < 0) || /*1*/
					((favourites.list[m].flags & NTFS_FLAG) != NTFS_FLAG && /*2*/
					((favourites.list[m].flags & GAMELIST_FILTER) > (list[n].flags & GAMELIST_FILTER)
					|| (list[n].flags & NTFS_FLAG) == NTFS_FLAG)) ||
					((favourites.list[m].flags & NTFS_FLAG) == NTFS_FLAG && (list[n].flags & D_FLAG_HDD0) == D_FLAG_HDD0)) /*3*/
				{
