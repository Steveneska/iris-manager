...
#define INSTALL_HVSC_REDIRECT(hvcall) uint64_t original_syscall_code_1 = peekq(HVSC_SYSCALL_ADDR_355DEX); \
	uint64_t original_syscall_code_2 = peekq(HVSC_SYSCALL_ADDR_355DEX + 8); \
	uint64_t original_syscall_code_3 = peekq(HVSC_SYSCALL_ADDR_355DEX + 16); \
	uint64_t original_syscall_code_4 = peekq(HVSC_SYSCALL_ADDR_355DEX + 24); \
	poke813(HVSC_SYSCALL_ADDR_355DEX, 0x7C0802A6F8010010ULL);	\
...
#define REMOVE_HVSC_REDIRECT() poke813(HVSC_SYSCALL_ADDR_355DEX, original_syscall_code_1); \
	...
	poke813(HVSC_SYSCALL_ADDR_355DEX + 24, original_syscall_code_4);

int lv1_insert_htab_entry_355dex(uint64_t htab_id, uint64_t hpte_group, uint64_t hpte_v, uint64_t hpte_r, uint64_t bolted_flag, uint64_t flags, uint64_t *hpte_index, uint64_t *hpte_evicted_v, uint64_t *hpte_evicted_r);
int lv1_allocate_memory_355dex(uint64_t size, uint64_t page_size_exp, uint64_t flags, uint64_t *addr, uint64_t *muid);
int lv1_undocumented_function_114_355dex(uint64_t start, uint64_t page_size, uint64_t size, uint64_t *lpar_addr);
void lv1_undocumented_function_115_355dex(uint64_t lpar_addr);

uint64_t lv2_alloc_355dex(uint64_t size, uint64_t pool);
