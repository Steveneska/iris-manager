...
#include <ppu-lv2.h>
#include "payload.h"
#include "payload_465dex.h"
#include "syscall8.h"

#include "payload_sky_465dex_bin.h"
#include "umount_465dex_bin.h"
...
#define UMOUNT_SYSCALL_OFFSET           (0x1ABB0C+0x8) // SYSCALL (838)
#define LV2MOUNTADDR_465dex 0x80000000004A36D0ULL
//0xff0 => 0x116c (458098 - 459204)
#define LV2MOUNTADDR_465dex_ESIZE 0x118
#define LV2MOUNTADDR_465dex_CSIZE 0x108
...
static int lv2_unpatch_bdvdemu_465dex(void);
static int lv2_patch_bdvdemu_465dex(uint32_t flags);
static int lv2_patch_storage_465dex(void);
static int lv2_unpatch_storage_465dex(void);
...
int is_firm_465dex(void)
{
    // TOC 4.65
   u64 toc;
   toc =peekq(0x8000000000003000ULL);
   if(toc == 0x8000000000375510ULL && peekq(0x800000000031EBA8ULL)!=0x323031342F31312FULL)
   {
      return 1;
   }
   else
...
int is_payload_loaded_465dex(void)
{
...
void set_bdvdemu_465dex(int current_payload)
{
    lv2_unpatch_bdvdemu = lv2_unpatch_bdvdemu_465dex;
    lv2_patch_bdvdemu   = lv2_patch_bdvdemu_465dex;
    lv2_patch_storage = lv2_patch_storage_465dex;
    lv2_unpatch_storage = lv2_unpatch_storage_465dex;
}
...
static inline void install_lv2_memcpy()
{
    ...
    /* This does not work on some PS3s */
        pokeq(NEW_POKE_SYSCALL_ADDR + 0x00, 0x4800000428250000ULL);
        pokeq(NEW_POKE_SYSCALL_ADDR + 0x08, 0x4182001438a5ffffULL);
        pokeq(NEW_POKE_SYSCALL_ADDR + 0x10, 0x7cc428ae7cc329aeULL);
        pokeq(NEW_POKE_SYSCALL_ADDR + 0x18, 0x4bffffec4e800020ULL);
        usleep(5000);
    }
}
...
void load_payload_465dex (int mode)
{
    ...
    lv2_memcpy(0x8000000000000000ULL + (u64) PAYLOAD_OFFSET,
                   (u64) payload_sky_465dex_bin,
                   payload_sky_465dex_bin_size);
...
*/
    ...
    _poke32(0x5E02C, 0x60000000);
    _poke32(0x5E040, 0x60000000);
    ...
    _poke(0x2764FC, 0x386000007C6307B4); //
    _poke32(0x2764FC + 8, 0x4E800020);  //
    ...
    */
    ...
    PATCH_JUMP(0x2BB034, (PAYLOAD_OFFSET+0x30)); // patch openhook
    _poke32(0x2BB010, 0xF821FF61); // free openhook Rogero 4.30 (put "stdu    %sp, -0xA0(%sp)" instead   "b       sub_2E9F98")
...
static int lv2_patch_storage_465dex(void)
{
    ...
    save_lv2_storage_patch= peekq(0x800000000030E460ULL);
    pokeq32(0x800000000030E460ULL, 0x40000000);
...
static int lv2_unpatch_storage_465dex(void)
{
    ...
    pokeq(0x800000000030E460ULL, save_lv2_storage_patch);
...
static int lv2_unpatch_bdvdemu_465dex(void)
{
    ...
    sys8_memcpy((u64) mem, LV2MOUNTADDR_465dex, 0x10 * 0x118);
    sys8_memcpy((u64) (mem + 0x1200), 0x80000000007EF020ULL , LV2MOUNTADDR_465dex_CSIZE);

    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_465dex_ESIZE)
    {
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD1", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            if(!memcmp(mem + n + 0x69, "temp_bdvd", 10))
            {
                sys8_memcpy(LV2MOUNTADDR_465dex + n + 0x69, (u64) "dev_bdvd\0", 10);
                flag++;
            }
        }
        ...
        if(!memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            {
                sys8_memcpy(LV2MOUNTADDR_465dex + n + 0x69, (u64) (mem + n + 0x79), 11);
                sys8_memset(LV2MOUNTADDR_465dex + n + 0x79, 0ULL, 12);
                flag += 10;
            }
        }
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD0", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
              ...
              && !memcmp(mem + n + 0x79, "esp_bdvd", 9) && peekq(0x80000000007EF000ULL)!=0)
           {
                ...
                sys8_memcpy(LV2MOUNTADDR_465dex + (u64) (n - 0x10), (u64) (mem + 0x1200) , (u64) LV2MOUNTADDR_465dex_CSIZE);
                flag += 10;
           }
        }
    }
...
static int lv2_patch_bdvdemu_465dex(uint32_t flags)
{
    ...
    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_465dex_ESIZE)
    {
            ...
            && !memcmp(mem + n + 0x69, "dev_bdvd", 9) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            if(usb >= 0)
                sys8_memcpy(LV2MOUNTADDR_465dex + n + 0x69, (u64) "temp_bdvd", 10);
        ...
        {
            if(noBDVD) pos = -1;
            sys8_memcpy(LV2MOUNTADDR_465dex + n + 0x69, (u64) "dev_bdvd\0\0", 11);
            sys8_memcpy(LV2MOUNTADDR_465dex + n + 0x79, (u64) &path_name[128], 11);
    ...
    if(pos > 0 && pos2 > 0)
    {
      ...
      memcpy(mem + 0x1220, mem + pos2 - 0x10, LV2MOUNTADDR_465dex_CSIZE);
      dat = LV2MOUNTADDR_465dex + (u64) (pos2 - 0x10);
      ...
      n = (int) LV2MOUNTADDR_465dex_CSIZE; // CDATAS
      memcpy(mem + 0x1214, &n, 0x4);

      memcpy(mem + pos2, mem + pos, LV2MOUNTADDR_465dex_CSIZE - 0x10);
      memcpy(mem + pos2 + 0x69, "dev_bdvd\0\0", 11);
      memcpy(mem + pos2 + 0x79, "esp_bdvd\0\0", 11);
      memset(mem + pos2 + 0xa4, 0, 8);

      sys8_memcpy(0x80000000007EF000ULL , ((u64) mem + 0x1200), LV2MOUNTADDR_465dex_CSIZE + 0x20);
      sys8_memcpy(LV2MOUNTADDR_465dex + (u64) pos2, ((u64) (mem + pos2)), (u64) (LV2MOUNTADDR_465dex_CSIZE - 0x10));
