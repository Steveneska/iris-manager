...
#include <ppu-lv2.h>
#include "payload.h"
#include "payload_465.h"
#include "syscall8.h"

#include "payload_sky_465_bin.h"
#include "umount_465_bin.h"
...
#define NEW_POKE_SYSCALL_ADDR           0x80000000001A6F44ULL  // Done // where above syscall is in lv2

#define PAYLOAD_OFFSET                  0x3d90
#define PERMS_OFFSET                    0x3560

#define PAYLOAD_UMOUNT_OFFSET           (0x3d90+0x400)
#define UMOUNT_SYSCALL_OFFSET           (0x1A565C + 0x8) // SYSCALL (838) fixed by zar //former offet by Habib (0x1B480 +0x8) // SYSCALL (838)

#define LV2MOUNTADDR_465 0x8000000000479FA0ULL // Done
//0xff0 => 0x116c (458098 - 459204)
#define LV2MOUNTADDR_465_ESIZE 0x118
#define LV2MOUNTADDR_465_CSIZE 0x108
...
static int lv2_unpatch_bdvdemu_465(void);
static int lv2_patch_bdvdemu_465(uint32_t flags);
static int lv2_patch_storage_465(void);
static int lv2_unpatch_storage_465(void);
...
{
    // TOC 4.65 (same as 4.66)
   u64 toc;
   toc =peekq(0x8000000000003000ULL);
   if(toc == 0x800000000034F960ULL)
   {
      return (peekq(0x80000000002FC938ULL)!=0x323031342F31312FULL); //timestamp: 2014/08
   }
   else
   {
      return 0;
...
int is_firm_466(void)
{
    // TOC 4.66 (same as 4.65)
   u64 toc;
   toc =peekq(0x8000000000003000ULL);
   if(toc == 0x800000000034F960ULL)
   {
      return (peekq(0x80000000002FC938ULL)==0x323031342F31312FULL); //timestamp: 2014/11
   }
   else
   {
      return 0;
...
int is_payload_loaded_465(void)
{
...
void set_bdvdemu_465(int current_payload)
{
    lv2_unpatch_bdvdemu = lv2_unpatch_bdvdemu_465;
    lv2_patch_bdvdemu   = lv2_patch_bdvdemu_465;
    lv2_patch_storage = lv2_patch_storage_465;
    lv2_unpatch_storage = lv2_unpatch_storage_465;
}
...
static int lv2_patch_storage_465(void)
{
    ...
    save_lv2_storage_patch= peekq(0x80000000002ECB18ULL);
    pokeq32(0x80000000002ECB18ULL, 0x40000000);
...
static int lv2_unpatch_storage_465(void)
{
    ...
    pokeq(0x80000000002ECB18ULL, save_lv2_storage_patch);
...
static int lv2_unpatch_bdvdemu_465(void)
{
    ...
    sys8_memcpy((u64) mem, LV2MOUNTADDR_465, 0x10 * 0x118);
    sys8_memcpy((u64) (mem + 0x1200), 0x80000000007EF020ULL , LV2MOUNTADDR_465_CSIZE);

    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_465_ESIZE)
    {
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD1", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            if(!memcmp(mem + n + 0x69, "temp_bdvd", 10))
            {
                sys8_memcpy(LV2MOUNTADDR_465 + n + 0x69, (u64) "dev_bdvd\0", 10);
                flag++;
            }
        }
        ...
        if(!memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            {
                sys8_memcpy(LV2MOUNTADDR_465 + n + 0x69, (u64) (mem + n + 0x79), 11);
                sys8_memset(LV2MOUNTADDR_465 + n + 0x79, 0ULL, 12);
                flag += 10;
            }
        }
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD0", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
              ...
              && !memcmp(mem + n + 0x79, "esp_bdvd", 9) && peekq(0x80000000007EF000ULL)!=0)
           {
                ...
                sys8_memcpy(LV2MOUNTADDR_465 + (u64) (n - 0x10), (u64) (mem + 0x1200) , (u64) LV2MOUNTADDR_465_CSIZE);
                flag += 10;
           }
        }
    }
...
static int lv2_patch_bdvdemu_465(uint32_t flags)
{
    ...
    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_465_ESIZE)
    {
            ...
            && !memcmp(mem + n + 0x69, "dev_bdvd", 9) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            if(usb >= 0)
                sys8_memcpy(LV2MOUNTADDR_465 + n + 0x69, (u64) "temp_bdvd", 10);
        ...
        {
            if(noBDVD) pos = -1;
            sys8_memcpy(LV2MOUNTADDR_465 + n + 0x69, (u64) "dev_bdvd\0\0", 11);
            sys8_memcpy(LV2MOUNTADDR_465 + n + 0x79, (u64) &path_name[128], 11);
    ...
    if(pos > 0 && pos2 > 0)
    {
      ...
      memcpy(mem + 0x1220, mem + pos2 - 0x10, LV2MOUNTADDR_465_CSIZE);
      dat = LV2MOUNTADDR_465 + (u64) (pos2 - 0x10);
      ...
      n = (int) LV2MOUNTADDR_465_CSIZE; // CDATAS
      memcpy(mem + 0x1214, &n, 0x4);

      memcpy(mem + pos2, mem + pos, LV2MOUNTADDR_465_CSIZE - 0x10);
      memcpy(mem + pos2 + 0x69, "dev_bdvd\0\0", 11);
      memcpy(mem + pos2 + 0x79, "esp_bdvd\0\0", 11);
      memset(mem + pos2 + 0xa4, 0, 8);

      sys8_memcpy(0x80000000007EF000ULL , ((u64) mem + 0x1200), LV2MOUNTADDR_465_CSIZE + 0x20);
      sys8_memcpy(LV2MOUNTADDR_465 + (u64) pos2, ((u64) (mem + pos2)), (u64) (LV2MOUNTADDR_465_CSIZE - 0x10));
