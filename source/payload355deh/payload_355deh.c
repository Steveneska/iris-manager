...
#include <ppu-lv2.h>
#include "payload.h"
#include "payload_355deh.h"
#include "syscall8.h"
#include "mm_355deh.h"
#include "hvcall_355deh.h"

#include "payload_sky_355deh_bin.h"
#include "umount_355deh_bin.h"
...
#define SYSCALL_BASE                    0x8000000000393A88ULL //3.55 DEH WOW!
#define NEW_POKE_SYSCALL                813
#define NEW_POKE_SYSCALL_ADDR           0x800000000019CB50ULL   // SYSCALL (813) done
#define PAYLOAD_OFFSET                  0xF590
#define PERMS_OFFSET                    0xEE38

#define PAYLOAD_UMOUNT_OFFSET           (0xF590+0x400)
#define UMOUNT_SYSCALL_OFFSET           (0x19A42C + 0x8) // SYSCALL (838) + 8 -> peek(SYSCALL_BASE + 838*8) -> peek(0x363B60 + 0x1A30)
#define LV2MOUNTADDR_355deh 0x80000000004444C4ULL
#define LV2MOUNTADDR_355deh_ESIZE 0x100
#define LV2MOUNTADDR_355deh_CSIZE 0xF0
...
static int lv2_unpatch_bdvdemu_355deh(void);
static int lv2_patch_bdvdemu_355deh(uint32_t flags);
static int lv2_patch_storage_355deh(void);
static int lv2_unpatch_storage_355deh(void);
...
int is_firm_355deh(void)
{
    // TOC 3.55 DECR
   u64 toc;
   toc = peekq(0x8000000000003000ULL);
   if(toc == 0x800000000037C6B0ULL)
   {
      return 1;
   }
   else
...
int is_payload_loaded_355deh(void)
{
...
void set_bdvdemu_355deh(int current_payload)
{
    lv2_unpatch_bdvdemu = lv2_unpatch_bdvdemu_355deh;
    lv2_patch_bdvdemu   = lv2_patch_bdvdemu_355deh;
    lv2_patch_storage = lv2_patch_storage_355deh;
    lv2_unpatch_storage = lv2_unpatch_storage_355deh;
}
...
static inline void remove_lv2_memcpy()
{
    ...
    //remove_new_poke();
        ...
        pokeq(NEW_POKE_SYSCALL_ADDR + 16, 0xEBC2FFB87C7F1B78ULL);
...
{
    int result = lv1_undocumented_function_114_355deh(0, 0xC, HV_SIZE_355DEH, &mmap_lpar_addr);
    if (result != 0)
    {
        return 0;
    }

    result = mm_map_lpar_memory_region_355deh(mmap_lpar_addr, HV_BASE_355DEH, HV_SIZE_355DEH, 0xC, 0);
    if (result)
    {
		lv1_undocumented_function_115_355deh(mmap_lpar_addr);
		mmap_lpar_addr = 0;
...
static int lv2_unpatch_bdvdemu_355deh(void)
{
    ...
    sys8_memcpy((u64) mem, LV2MOUNTADDR_355deh, 0x10 * 0x100);
    sys8_memcpy((u64) (mem + 0x1200), 0x80000000007EF020ULL , LV2MOUNTADDR_355deh_CSIZE);

    for(n = 0; n < 0xff0; n+= LV2MOUNTADDR_355deh_ESIZE)
    {
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD1", 21) && mem[n-9]== 1 && mem[n-5]== 1)
        {
            if(!memcmp(mem + n + 0x69, "temp_bdvd", 10))
            {
                sys8_memcpy(LV2MOUNTADDR_355deh + n + 0x69, (u64) "dev_bdvd\0", 10);
                flag++;
            }
        }
        ...
        if(!memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29) && mem[n-9]== 1 && mem[n-5]== 1)
        {
            ...
            {
                sys8_memcpy(LV2MOUNTADDR_355deh + n + 0x69, (u64) (mem + n + 0x79), 11);
                sys8_memset(LV2MOUNTADDR_355deh + n + 0x79, 0ULL, 12);
                flag += 10;
           }
        }
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD0", 21) && mem[n-9]== 1 && mem[n-5]== 1)
        {
              ...
              && !memcmp(mem + n + 0x79, "esp_bdvd", 9) && peekq(0x80000000007EF000ULL)!=0)
           {
                ...
                sys8_memcpy(LV2MOUNTADDR_355deh + (u64) (n - 0xc), (u64) (mem + 0x1200) , (u64) LV2MOUNTADDR_355deh_CSIZE);
                flag += 10;
           }
        }
    }
...
static int lv2_patch_bdvdemu_355deh(uint32_t flags)
{
    ...
    for(n = 0; n < 0xff0; n+= LV2MOUNTADDR_355deh_ESIZE)
    {
            ...
            && !memcmp(mem + n + 0x69, "dev_bdvd", 9) && mem[n-9]== 1 && mem[n-5]== 1)
        {
            ...
            if(usb >= 0)
                sys8_memcpy(LV2MOUNTADDR_355deh + n + 0x69, (u64) "temp_bdvd", 10);
        ...
        {
            if(noBDVD) pos = -1;
            sys8_memcpy(LV2MOUNTADDR_355deh + n + 0x69, (u64) "dev_bdvd\0\0", 11);
            sys8_memcpy(LV2MOUNTADDR_355deh + n + 0x79, (u64) &path_name[128], 11);
    ...
    if(pos > 0 && pos2 > 0)
    {
      ...
      memcpy(mem + 0x1220, mem + pos2 - 0xc, LV2MOUNTADDR_355deh_CSIZE);
      dat = LV2MOUNTADDR_355deh + (u64) (pos2 - 0xc);
      ...
      n = (int) LV2MOUNTADDR_355deh_CSIZE; // CDATAS
      memcpy(mem + 0x1214, &n, 0x4);

      memcpy(mem + pos2, mem + pos, LV2MOUNTADDR_355deh_CSIZE - 0xc);
      memcpy(mem + pos2 + 0x69, "dev_bdvd\0\0", 11);
      memcpy(mem + pos2 + 0x79, "esp_bdvd\0\0", 11);
      memset(mem + pos2 + 0xa4, 0, 8);

      sys8_memcpy(0x80000000007EF000ULL , ((u64) mem + 0x1200), LV2MOUNTADDR_355deh_CSIZE + 0x20);
      sys8_memcpy(LV2MOUNTADDR_355deh + (u64) pos2, ((u64) (mem + pos2)), (u64) (LV2MOUNTADDR_355deh_CSIZE - 0xc));
