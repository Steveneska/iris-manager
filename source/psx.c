...
void draw_psx_options(float x, float y, int index)
{
	...
	if(new_pad & (BUTTON_CROSS_))
	{
		switch(select_option)
		{
			...
			case 7: // Delete from Favorites / Copy to Favorites
...
u8 * build_from_cue(char *path, int *len, u32 last_sec)
{
	...
	if(error)
	{
		...
		{
			case 1:
				DrawDialogOKTimer("CUE sheet error: MODE2/AUDIO must be first token after track data.", 2000.0f);
				break;
			case 2:
				DrawDialogOKTimer("CUE sheet error: Two MODES detected in track data.", 2000.0f);
				break;
			case 3:
				DrawDialogOKTimer("CUE sheet error: INDEX 01 found before MODE token.", 2000.0f);
				break;
			case 4:
				DrawDialogOKTimer("CUE sheet error: INDEX 01 is not valid.", 2000.0f);
				break;
			case 5:
				DrawDialogOKTimer("CUE sheet error: Invalid track data order.", 2000.0f);
			case 6:
				DrawDialogOKTimer("CUE sheet error: Incomplete track data found.", 2000.0f);
				break;
			case 7:
				DrawDialogOKTimer("CUE sheet error: Invalid PREGAP.", 2000.0f);
				break;
		}
...
int psx_iso_prepare(char *path, char *name, char *isopath)
{
	...
	{
		// Test old_psxemu use when CD is ejected under Cobra plugin
		struct stat s;
...
skip_scan_folder_isos:
		...
		if(nfiles > 0)
		{
			...
			if(nfiles < 8 && forced_no_cd != 2)
			{
				...
				if(are_using_cheats == 2)
				{
					...
					if(DrawDialogYesNo(language[DRAWPSX_CHEATMAKE]) == YES)
					{
						...
						{
							memcpy(files[nfiles], files[8], MAXPATHLEN); // files[8] return doesn't have the same name
							nfiles++;
							int m;
							are_using_cheats = 1;
							for(m = 0; m < nfiles - 1; m++)
			...
			if(forced_no_cd == 2)
			{
				// NTFS/EXT (copy partial ISO to HDD0)
			...
			for(n = 0; n < 8; n++)
			{
				...
				if(forced_no_cd == 2) add_sys8_path_table(files[8], files[7]); // NTFS/EXT without CD
			...
			else
			{
				...
				{
					// use NTFS/EXT if it is connected
					for(n = 0; n < 8; n++)
					{
	...
	use_hdd0:
			...
			if(!strcmp(psx_options.mc1, "No Memory Card") || !strcmp(psx_options.mc1, "Internal_MC.VM1"))
			{
				...
				{
					if(!strncmp(path, "/dev_usb", 8) || !strncmp(path, "/ntfs", 5) || !strncmp(path, "/ext", 4))
					{
						// Move the MC from USB to the main folder
...
void unload_psx_payload()
{
	...
	if(value == 0x45505331454D5531ULL)
	{
		...
		value = lv2peek(0x80000000000018A0ULL); // 604 (send syscall base command)
...
void load_psx_payload()
{
	...
	if(base1) {
		sys8_pokeinstr(0x80000000000018A0ULL, base2); // 604 (send syscall base command)
...
void psx_launch(void)
{
	...
	if((psx_options.flags & PSX_PAYLOAD) == PSX_PAYLOAD) psx_options.flags ^= PSX_PAYLOAD; //  Don't use flag 2 here
