/* Called from file_manager.c */
{
	...
	if(ROOT_MENU)
	{
		...
		display_ttf_string(0, py,	(mount_option == 1) ? "Mount /net_host0" :
									...
									!dev_blind ? "Mount Flash Memory" : "Unmount Flash Memory", blink ? INVISIBLE : WHITE, 0, 16, 24); py += 24;
	...
	else
	{
		...
		{
			if((!fm_pane && isDir(path1)) || (fm_pane && isDir(path2)))
				{display_ttf_string(0, py, "Archive Folder",  (set_menu2 == 3 && (frame & BLINK)) ? INVISIBLE : WHITE, 0, 16, 24); py += 24;}
			else
				copy_mode = 0;
		}
		else if(copy_mode == 0)
		...
		display_ttf_string(0, py, ((!fm_pane && (entries1[sel1].d_type & IS_DIRECTORY)) || ( fm_pane && (entries2[sel2].d_type & IS_DIRECTORY))) ?
								"Create ISO from Folder" : "Create ISO from File",
														(set_menu2 == 7 && (frame & BLINK)) ? INVISIBLE : WHITE, 0, 16, 24); py += 24;

		display_ttf_string(0, py, ((!fm_pane && (entries1[sel1].d_type & IS_DIRECTORY)) || ( fm_pane && (entries2[sel2].d_type & IS_DIRECTORY))) ?
								"Display Folder Info" : "Display File Info",
														(set_menu2 == 8 && (frame & BLINK)) ? INVISIBLE : WHITE, 0, 16, 24); py += 24;

		if(is_dir)
			{display_ttf_string(0, py, (mnt_mode == 1) ? "Mount and Exit" : (mnt_mode == 2) ? "Fix Permissions" : "Mount Folder",
		...
		else if(max_menu2 >= 9)
		{
			if( (!fm_pane && (!strcmpext(entries1[sel1].d_name, ".iso") || !strcmpext(entries1[sel1].d_name, ".iso.0"))) ||
				...
				{display_ttf_string(0, py, (mnt_mode == 1) ? "Mount and Exit" : (mnt_mode == 2) ? "Create Dummy ISO" : "Mount as Blu-Ray Disc",
														(set_menu2 == 9 && (frame & BLINK)) ? INVISIBLE : WHITE, 0, 16, 24); py += 24;}
			else
				{display_ttf_string(0, py, "Fix Game",	(set_menu2 == 9 && (frame & BLINK)) ? INVISIBLE : WHITE, 0, 16, 24); py += 24;}
		}
