...
bool options_locked = false; // True when control parental is between 1 and 9, false for parental 0
...
int game_list_category = GAME_LIST_PS3_ONLY;
...
static void do_umount_iso(void)
{
	...
	// If there is a disc in the system, it must be ejected
...
void fun_exit()
{
	...
	if(!is_ps3game_running && !use_cobra && lv2peek(0x80000000000004E8ULL)) syscall_40(1, 0); // Disable PS3 disc-less
...
void auto_ftp(void)
{
	...
	one = 0;
	if (ftp_serv || (manager_cfg.opt_flags & OPTFLAGS_FTP))
	{
		manager_cfg.opt_flags |= OPTFLAGS_FTP;

		int r = ftp_init();

		if(r == SUCCESS) ftp_inited = true;
		else
			show_ftp_error(r);
	}
...
void video_adjust()
{
	...
	while(true)
	{
		...
		DrawFormatString(0, (512 - 24)/2 + 128, "%s", "Press [] for English language");
...
bool test_ftp_working()
{
	...
	if(get_ftp_activity())
	{
		if(DrawDialogYesNoDefaultYes("The FTP is currently in use.\n"
									 "Do you want to end the FTP server task?") == YES)
		{
			ftp_deinit();
			ftp_inited = false;
			return false;
...
void read_settings()
{
	...
	char BackgroundGears[2] = "0";
	char IconPulse[2] = "0";
	char FileManager[2] = "0";
	char TimeFormat[2] = "4";
	char UnmountDevBlind[2] = "1";
	...
	else if(file_exists("/dev_hdd0/game/SSNE10000/USRDIR/EBOOT.BIN"))
		strcpy(retroarch_path, "/dev_hdd0/game/SSNE10000");
	else
		strcpy(retroarch_path, "/dev_hdd0/game/SSNE10001");
	strcpy(ps2classic_path, "/PS2ISO");
	sprintf(psxiso_path, "%s/USRDIR/PS1ISO", self_path);
	...
	strcpy(video_extensions, ".MKV .MP4");
	strcpy(audio_extensions, ".MP3 .WAV .WMA .AAC");
	strcpy(rom_extensions, ".ZIP");
	strcpy(custom_homebrews, "IRISMAN00|");
	...
	if(file)
	{
		...
		mod_volume = getConfigMemValueInt((char *) file, file_size, GUI_SECTION, "ModVolume", 0);
...
s32 main(s32 argc, const char* argv[])
{
	...
	if(lv2peek(0x80000000000004E8ULL)) syscall_40(1, 0); // Disable PS3 disc-less
	...
	segments = (sysSpuSegment*)memalign(128, SPU_SIZE(segmentsize)); // Must be aligned to 128 or it will break malloc() allocations
	...
	// This code should not be removed from here to avoid black screen caused by the patches (e.g. cache problems)
	...
	manager_cfg.noBDVD = (use_cobra || use_mamba || file_exists(tmp_path)) ? MODE_DISCLESS : MODE_WITHBDVD; // Set default to DISCLESS only for Cobra/Mamba firmware
	...
	// Load config and language strings
	LoadManagerCfg();
	...
	if(use_mamba && !use_cobra)
	{
		...
		// Relaunch to get VSH process for Mamba
		sprintf(tmp_path, "%s/USRDIR/RELOAD.SELF", self_path);
	...
	if(mem)
	{
		...
		if(strstr(directories[currentgamedir].path_name, "/ntfs"))
		{
			...
			// NTFS/EXT
			if(use_cobra && mode_homebrew != HOMEBREW_MODE)
			{
	...
	skip_bdvd:
				// Limit the selectable devices to 3
				...
				if(find_device == BDVD_DEVICE)
				{
					if(((fdevices>>BDVD_DEVICE) != (fdevices_old>>BDVD_DEVICE) || ((forcedevices>>find_device) & 1)))
					{
						...
						else
						{
							...
							bluray_game[0] = 0;
							// Unused if statement -- remove later?
				...
				if(fdevices != fdevices_old || ((forcedevices>>find_device) & 1))
				{
					...
					// BDISO, DVDISO, MKV
					if(mode_homebrew == VIDEOS_MODE && find_device == HDD0_DEVICE && ((fdevices>>find_device) & 1))
					{
						// ISO BR-DVD
						char file[0x420];
...
skip_refresh:
		...
		if(found_game_insert || found_game_remove)
		{
			...
			mode_favourites = mode_favourites != 0; // Avoid insert favorites
...
void load_background_picture()
{
	if(bk_picture)
	{
		if(bk_picture == BG_RANDOM || bk_picture == BG_PIC1)
		{
			int retry = 0, bg;
...
void load_gamecfg(int current_dir)
{
	...
	last_selected = current_dir; // Prevent load again
	bShowPath = false;
...
int save_gamecfg()
{
	...
	// Save Game Launcher game config
...
void draw_grid(float x, float y)
{
	...
	if((frame_count & 0x100))
	{
		...
		x2 = DrawFormatString(1024, 0, " CPU: 99ºC RSX: 99ºC ");

		y2 = y + 3 * 150 - 4 + 12;
		SetFontColor(WHITE, BLACK);
		x2 = DrawFormatString(x + 4 * 200 - (x2 - 1024) - 12 , y2, " CPU: ");
	...
	for(l = 0; l < 2; l++)
	{
		...
		for(n = 0; n < 3; n++)
			for(m = 0; m < 4; m++)
			{
				...
				{
					// Draw Blu-Ray icon with empty icon
	...
	if((Png_offset[i] && !mode_favourites) || (mode_favourites && favourites.list[i].title_id[0] != 0))
	{
		...
		else
		{
			...
			if(!(directories[get_currentdir(i)].flags & D_FLAG_PS3_ISO))
			{
				...
				x2 = DrawString(x + x2,  y + 3 * 150 - 48 + 4, (directories[get_currentdir(i)].flags & (PS1_FLAG)) == (PS1_FLAG) ? "CD-ROM" :
			...
			else if((directories[get_currentdir(i)].flags & (PSP_FLAG | RETRO_FLAG | PS2_CLASSIC_FLAG)) == (PSP_FLAG | RETRO_FLAG | PS2_CLASSIC_FLAG))
			{
				...
				x2 = DrawString(x + x2,  y + 3 * 150 - 48 + 4, (directories[get_currentdir(i)].flags & (PS2_FLAG)) == (PS2_FLAG) ? "DVD-ROM" : "Blu-Ray Disc");
...
int gui_control()
{
	...
	if(new_pad & BUTTON_TRIANGLE)
	{
		...
		{
			if (old_pad & BUTTON_SELECT) { bShowVersion = 1; bShowPIC1 = 1; } // [SELECT] + [TRIANGLE] = Force show version / PIC1 on ISO
...
void return_to_game_list(bool update)
{
	...
	menu_screen = SCR_MAIN_GAME_LIST;
	select_option = 0;
	frame_count = 0xFF; // Force display temperature
}
...
void draw_background_pic1()
{
	...
	if((bLoadPIC1 && (bk_picture == BG_PIC1) && frame_count > 55) && Png_offset[selected])
	{
		int i = selected;
		bLoadPIC1 = false;
		...
		if((bShowPIC1 == 0 && bk_picture == BG_PIC1) || (bShowPIC1 == 1) || (bShowPIC1 == 2 && ((directories[indx].flags & (NTFS_FLAG)) != NTFS_FLAG)))
		{
			// Program new event thread function to show background picture
			event_thread_send(0x555ULL, (u64) get_pict, (u64) &i);
		}
...
void draw_device_cpyiso(float x, float y, int index)
{
	...
	if(new_pad & BUTTON_CROSS_)
	{
		...
		{
			DrawDialogOKTimer("Error:! Cannot copy in the same device", 2000.0f);
			menu_screen = SCR_MENU_ISO_OPTIONS;
			select_option = 3;
		}
...
void draw_options(float x, float y, int index)
{
	if(strlen(directories[currentgamedir].title_id) == 9 && strstr("IRISMAN00", directories[currentgamedir].title_id) != NULL)
	{
...
void draw_configs(float x, float y, int index)
{
	...
	x2 = DrawButton1_UTF8(x + 32, y2, 320, "Fix Permissions", (flash && (select_option == 0))) + 16; // Do no translate this
	...
	if(manager_cfg.global_wm_autoplay)
		x2 = DrawButton2_UTF8(x2, y2, 0, "Global Auto-Play", true) + 8;
	else
		x2 = DrawButton2_UTF8(x2, y2, 0, "Auto-Play"  , (game_cfg.direct_boot == 3)) + 8;
...
void draw_gbloptions(float x, float y)
{
	...
	sprintf(temp_buffer, "%s: %s", language[DRAWGLOPT_CHANGEGUI], (gui_mode == MODE_COVERFLOW) ? "CoverFlow" : "CoverFlow");
	...
	if (net_option == 0)
		...
		DrawButton1_UTF8(box_left, y2, 520, (net_option ==  1) ? "Mount /net_host0/PKG as /dev_bdvd" :
											...
											(net_option ==  9) ? (file_exists("/dev_blind") ? "/dev_blind : ON" : "/dev_blind : OFF") :
											(net_option == 10) ? "Rebuild Database" :
	...
	if(select_option == 1)
	{
		...
		DrawFormatString(500, y2 - 28, "[SQUARE] - Toggle Game Covers                [LEFT] / [RIGHT] - Change Display mode");
	}
...
exit_gbloptions:
		...
		((select_option <= 1) || ((select_option == 2) && (background_sel > 1)) || (select_option == 5)) ))
	{
		...
		switch(select_option)
		{
				...
				switch (net_option)
				{
					...
					case 2: // Mount net0/VIDEO
						if(bAllowNetGames && get_net_status() == SUCCESS)
						{
							...
							if(bk_picture == BG_PIC1) load_background_picture();
			...
			case 6: // Download Covers
				...
				wait_event_thread();
				get_games();
				if(n == 0)    DrawDialogOKTimer("Covers downloaded successfully", 2000.0f);
				if(n == -1)   DrawDialogOKTimer("Some covers could not be downloaded", 2000.0f);
				if(n == -555) DrawDialogOKTimer("Aborted by the user", 2000.0f);
				break;
	...
	{
		if((new_pad & BUTTON_SQUARE) && (ftp_inited == false) && (net_option == 0))
		{
...
void draw_toolsoptions(float x, float y)
{
		...
		(!(select_option == 4 || select_option == 5) && (new_pad & BUTTON_RIGHT)) )
	{
		switch(select_option)
		{
			...
			case 1: //PKG install
				DrawDialogOKTimer("For NTFS and EXT2/3/4 devices use the File Manager.", 5000.0f);
				draw_pkginstall(x, y);
				break;
...
void draw_gamelist_options(float x, float y)
{
	...
	DrawButton1_UTF8(x2, y2, 520, "Homebrew", (flash && (select_option == 4)));
...
int patch_bdvdemu(u32 flags)
{
	...
	{
		if(noBDVD == MODE_DISCLESS && !use_cobra) flags = NTFS_FLAG; else return SUCCESS; // New to add BD-Emu
	}
