...
void read_settings()
{
	...
	if(file)
	{
		...
		max_roms = getConfigMemValueInt((char *) file, file_size, ROMS_SECTION, "MaxRoms", 0);
...
exit_gbloptions:
	...
	{
		if((new_pad & BUTTON_SQUARE) && (ftp_inited == false/*(manager_cfg.opt_flags & OPTFLAGS_FTP) == 0*/) && (net_option == 0))
		{
