...
#include <ppu-lv2.h>
#include "payload.h"
#include "payload_470dex.h"
#include "syscall8.h"

#include "payload_sky_470dex_bin.h"
#include "umount_470dex_bin.h"
...
#define UMOUNT_SYSCALL_OFFSET           (0x2BACC0+0x8) // SYSCALL (838)
#define LV2MOUNTADDR_470dex 0x80000000004A3678ULL
//0xff0 => 0x116c (458098 - 459204)
#define LV2MOUNTADDR_470dex_ESIZE 0x118
#define LV2MOUNTADDR_470dex_CSIZE 0x108
...
static int lv2_unpatch_bdvdemu_470dex(void);
static int lv2_patch_bdvdemu_470dex(uint32_t flags);
static int lv2_patch_storage_470dex(void);
static int lv2_unpatch_storage_470dex(void);
...
int is_firm_470dex(void)
{
    ...
    if(addr == NEW_POKE_SYSCALL_ADDR && peekq(0x800000000000C4D0ULL) == 0x2F8300007F84E378ULL) return 1;
...
void set_bdvdemu_470dex(int current_payload)
{
    lv2_unpatch_bdvdemu = lv2_unpatch_bdvdemu_470dex;
    lv2_patch_bdvdemu   = lv2_patch_bdvdemu_470dex;
    lv2_patch_storage = lv2_patch_storage_470dex;
    lv2_unpatch_storage = lv2_unpatch_storage_470dex;
}
...
// Found @ 0x2BC5AC This is per FW, syscall_rmdir_desc // SC813
        ...
        pokeq(NEW_POKE_SYSCALL_ADDR + 16, 0xEBC22EB87C7F1B78ULL);
...
static int lv2_patch_storage_470dex(void)
{
    ...
    save_lv2_storage_patch= peekq(0x800000000030E4F8ULL);
    pokeq32(0x800000000030E4F8ULL, 0x40000000);
...
static int lv2_unpatch_storage_470dex(void)
{
    ...
    pokeq(0x800000000030E4F8ULL, save_lv2_storage_patch);
...
static int lv2_unpatch_bdvdemu_470dex(void)
{
    ...
    sys8_memcpy((u64) mem, LV2MOUNTADDR_470dex, 0x10 * 0x118);
    sys8_memcpy((u64) (mem + 0x1200), 0x80000000007EF020ULL , LV2MOUNTADDR_470dex_CSIZE);

    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_470dex_ESIZE)
    {
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD1", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            if(!memcmp(mem + n + 0x69, "temp_bdvd", 10))
            {
                sys8_memcpy(LV2MOUNTADDR_470dex + n + 0x69, (u64) "dev_bdvd\0", 10);
                flag++;
            }
        }
        ...
        if(!memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            {
                sys8_memcpy(LV2MOUNTADDR_470dex + n + 0x69, (u64) (mem + n + 0x79), 11);
                sys8_memset(LV2MOUNTADDR_470dex + n + 0x79, 0ULL, 12);
                flag += 10;
            }
        }
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD0", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
              ...
              && !memcmp(mem + n + 0x79, "esp_bdvd", 9) && peekq(0x80000000007EF000ULL)!=0)
           {
                ...
                sys8_memcpy(LV2MOUNTADDR_470dex + (u64) (n - 0x10), (u64) (mem + 0x1200) , (u64) LV2MOUNTADDR_470dex_CSIZE);
                flag += 10;
           }
        }
    }
...
static int lv2_patch_bdvdemu_470dex(uint32_t flags)
{
    ...
    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_470dex_ESIZE)
    {
            ...
            && !memcmp(mem + n + 0x69, "dev_bdvd", 9) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            if(usb >= 0)
                sys8_memcpy(LV2MOUNTADDR_470dex + n + 0x69, (u64) "temp_bdvd", 10);
        ...
        {
            if(noBDVD) pos = -1;
            sys8_memcpy(LV2MOUNTADDR_470dex + n + 0x69, (u64) "dev_bdvd\0\0", 11);
            sys8_memcpy(LV2MOUNTADDR_470dex + n + 0x79, (u64) &path_name[128], 11);
    ...
    if(pos > 0 && pos2 > 0)
    {
      ...
      memcpy(mem + 0x1220, mem + pos2 - 0x10, LV2MOUNTADDR_470dex_CSIZE);
      dat = LV2MOUNTADDR_470dex + (u64) (pos2 - 0x10);
      ...
      n = (int) LV2MOUNTADDR_470dex_CSIZE; // CDATAS
      memcpy(mem + 0x1214, &n, 0x4);

      memcpy(mem + pos2, mem + pos, LV2MOUNTADDR_470dex_CSIZE - 0x10);
      memcpy(mem + pos2 + 0x69, "dev_bdvd\0\0", 11);
      memcpy(mem + pos2 + 0x79, "esp_bdvd\0\0", 11);
      memset(mem + pos2 + 0xa4, 0, 8);

      sys8_memcpy(0x80000000007EF000ULL , ((u64) mem + 0x1200), LV2MOUNTADDR_470dex_CSIZE + 0x20);
      sys8_memcpy(LV2MOUNTADDR_470dex + (u64) pos2, ((u64) (mem + pos2)), (u64) (LV2MOUNTADDR_470dex_CSIZE - 0x10));
