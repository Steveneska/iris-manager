...
static int download_update(char *url, char *file, int mode, u64 *size)
{
	...
	if(mode == 1) goto err; // Get the size only
		...
		if(!game_up_mode) {
			...
			if(ret == 0)
			{
				...
				else
				{
					...
					msgDialogOpen2(mdialogyesno3, "Replace Left Cover with Right Cover?", my_dialog, (void*)  0x0000aaaa, NULL );
