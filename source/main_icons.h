...
int LoadTexturePNG(char * filename, int index)
{
	...
	if(index == num_box || index == BIG_PICT) texture_pointer2 += 2048 * 1200; // Reserve 2048x1200 x4 for background picture

	// More textures can be added here using 'texture_pointer'. It is returned aligned to 16 bytes
	...
	if(my_png_datas.bmp_out)
	{
		...
		memcpy(&Png_datas[index], &my_png_datas, sizeof(PngDatas));
		Png_offset[index] = tiny3d_TextureOffset(my_png_datas.bmp_out); // Get offset (RSX use offset instead of address)
...
int get_icon(char * path, const int num_dir)
{
	...
	if (!is_update)
	{
		...
		// Blu-Ray / DVD / MKV in Homebrew mode
...
void LoadTexture()
{
	...
	for(i = 0; i < MAX_RESOURCES; i++)
	{
		...
		if(Png_res[i].bmp_out)
		{
			...
			free(Png_res[i].bmp_out); // Free the PNG because we don't need this data

			Png_res_offset[i] = tiny3d_TextureOffset(texture_pointer); // Get offset (RSX use offset instead of address)
