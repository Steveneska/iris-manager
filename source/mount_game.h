...
void launch_ps2classic(char *ps2iso_path, char *ps2iso_title)
{
	if(file_exists(PS2_CLASSIC_PLACEHOLDER))
	{
		...
		if(ret)
			DrawDialogOKTimer("PlayStation®2 Format Disc could not be copied to PS2 Classics Placeholder", 5000.0f);
...
void launch_retro(char *rom_path)
{
	...
	{
		DrawDialogOKTimer("RetroArch is not installed.\nCannot launch the selected game.", 3000.0f);
		return;
	}
