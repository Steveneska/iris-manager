...
#include <ppu-lv2.h>
#include "payload.h"
#include "payload_480dex.h"
#include "syscall8.h"

#include "payload_sky_480dex_bin.h"
#include "umount_480dex_bin.h"
...
#define SYSCALL_BASE                    0x800000000038A4E8ULL // 4.80 DEX
#define NEW_POKE_SYSCALL                813
#define NEW_POKE_SYSCALL_ADDR           0x80000000002BC6F0ULL  // Syscall 813

#define PAYLOAD_OFFSET                  0x3d90
#define PERMS_OFFSET                    0x3560

#define PAYLOAD_UMOUNT_OFFSET           (0x3d90+0x400)
#define UMOUNT_SYSCALL_OFFSET           (0x2BAE04 + 0x8) // SYSCALL (838) + 8 -> peek(SYSCALL_BASE + 838*8) -> peek(0x363BE0 + 0x1A30)

#define LV2MOUNTADDR_480dex 0x80000000004A3678ULL
//0xff0 => 0x116c (458098 - 459204)
#define LV2MOUNTADDR_480dex_ESIZE 0x118
#define LV2MOUNTADDR_480dex_CSIZE 0x108
...
static int lv2_unpatch_bdvdemu_480dex(void);
static int lv2_patch_bdvdemu_480dex(uint32_t flags);
static int lv2_patch_storage_480dex(void);
static int lv2_unpatch_storage_480dex(void);
...
int is_firm_480dex(void)
{
   // TOC 4.80
   u64 toc;
   toc =peekq(0x8000000000003000ULL);
   if(toc == 0x80000000003759B0ULL)
   {
      return 1;
   }
   else
...
int is_payload_loaded_480dex(void)
{
...
void set_bdvdemu_480dex(int current_payload)
{
    lv2_unpatch_bdvdemu = lv2_unpatch_bdvdemu_480dex;
    lv2_patch_bdvdemu   = lv2_patch_bdvdemu_480dex;
    lv2_patch_storage   = lv2_patch_storage_480dex;
    lv2_unpatch_storage = lv2_unpatch_storage_480dex;
}
...
void load_payload_480dex(int mode)
{
/*
...
*/
    ...
    lv2_memcpy(0x8000000000000000ULL + (u64) PAYLOAD_OFFSET,
                   (u64) payload_sky_480dex_bin,
                   payload_sky_480dex_bin_size);

    lv2_memcpy(0x8000000000000000ULL + (u64) PAYLOAD_UMOUNT_OFFSET, // copy umount routine
                      (u64) umount_480dex_bin,
                      umount_480dex_bin_size);
    ...
    */
			...
			pokeq(0x800000000022DAC8ULL, 0x38600000F8690000ULL ); // fix 0x8001002B / 80010017 errors
    ...
    _poke32(0x5E0B0,  0x60000000);            // fix 80010009 error
    _poke32(0x5E0C4,  0x60000000);            // fix 80010019 error
    _poke(  0x59F58,  0x63FF003D60000000); // fix 8001003D error  "ori     %r31, %r31, 0x3D\n nop\n" done
    _poke32(0x5A020,  0x3BE00000);            // fix 8001003E error -- 3.55 ok in 0x055F64 "li      %r31, 0"  done
    ...
    */
    ...
    PATCH_JUMP(0x2B25E8, (PAYLOAD_OFFSET+0x30)); // patch openhook - done
...
static int lv2_patch_storage_480dex(void)
{
    ...
    save_lv2_storage_patch= peekq(0x800000000030E638ULL);
    pokeq32(0x800000000030E638ULL, 0x40000000); //FIXED
...
static int lv2_unpatch_storage_480dex(void)
{
    ...
    pokeq(0x800000000030E638ULL, save_lv2_storage_patch); //FIXED
...
static int lv2_unpatch_bdvdemu_480dex(void)
{
    ...
    sys8_memcpy((u64) mem, LV2MOUNTADDR_480dex, 0x10 * 0x118);
    sys8_memcpy((u64) (mem + 0x1200), 0x80000000007EF020ULL , LV2MOUNTADDR_480dex_CSIZE);

    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_480dex_ESIZE)
    {
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD1", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            if(!memcmp(mem + n + 0x69, "temp_bdvd", 10))
            {
                sys8_memcpy(LV2MOUNTADDR_480dex + n + 0x69, (u64) "dev_bdvd\0", 10);
                flag++;
            }
        }
        ...
        if(!memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            {
                sys8_memcpy(LV2MOUNTADDR_480dex + n + 0x69, (u64) (mem + n + 0x79), 11);
                sys8_memset(LV2MOUNTADDR_480dex + n + 0x79, 0ULL, 12);
                flag += 10;
            }
        }
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD0", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
              ...
              && !memcmp(mem + n + 0x79, "esp_bdvd", 9) && peekq(0x80000000007EF000ULL)!=0)
           {
                ...
                sys8_memcpy(LV2MOUNTADDR_480dex + (u64) (n - 0x10), (u64) (mem + 0x1200) , (u64) LV2MOUNTADDR_480dex_CSIZE);
                flag += 10;
           }
        }
    }
...
static int lv2_patch_bdvdemu_480dex(uint32_t flags)
{
    ...
    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_480dex_ESIZE)
    {
            ...
            && !memcmp(mem + n + 0x69, "dev_bdvd", 9) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            if(usb >= 0)
                sys8_memcpy(LV2MOUNTADDR_480dex + n + 0x69, (u64) "temp_bdvd", 10);
        ...
        {
            if(noBDVD) pos = -1;
            sys8_memcpy(LV2MOUNTADDR_480dex + n + 0x69, (u64) "dev_bdvd\0\0", 11);
            sys8_memcpy(LV2MOUNTADDR_480dex + n + 0x79, (u64) &path_name[128], 11);
    ...
    if(pos > 0 && pos2 > 0)
    {
      ...
      memcpy(mem + 0x1220, mem + pos2 - 0x10, LV2MOUNTADDR_480dex_CSIZE);
      dat = LV2MOUNTADDR_480dex + (u64) (pos2 - 0x10);
      ...
      n = (int) LV2MOUNTADDR_480dex_CSIZE; // CDATAS
      memcpy(mem + 0x1214, &n, 0x4);

      memcpy(mem + pos2, mem + pos, LV2MOUNTADDR_480dex_CSIZE - 0x10);
      memcpy(mem + pos2 + 0x69, "dev_bdvd\0\0", 11);
      memcpy(mem + pos2 + 0x79, "esp_bdvd\0\0", 11);
      memset(mem + pos2 + 0xa4, 0, 8);

      sys8_memcpy(0x80000000007EF000ULL , ((u64) mem + 0x1200), LV2MOUNTADDR_480dex_CSIZE + 0x20);
      sys8_memcpy(LV2MOUNTADDR_480dex + (u64) pos2, ((u64) (mem + pos2)), (u64) (LV2MOUNTADDR_480dex_CSIZE - 0x10));
