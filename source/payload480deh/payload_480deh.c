...
#include <ppu-lv2.h>
#include "payload.h"
#include "payload_480deh.h"
#include "syscall8.h"

#include "payload_sky_480deh_bin.h"
#include "umount_480deh_bin.h"
...
#define SYSCALL_BASE                    0x80000000003B28F8ULL // 4.80 DEH (same as 4.75 DEH)
#define NEW_POKE_SYSCALL                813
#define NEW_POKE_SYSCALL_ADDR           0x80000000002D2788ULL  // Syscall 813

#define PAYLOAD_OFFSET                  0x3d90
#define PERMS_OFFSET                    0x3560

#define PAYLOAD_UMOUNT_OFFSET           (0x3d90+0x400)
#define UMOUNT_SYSCALL_OFFSET           (0x2D0EA0 + 0x8) // SYSCALL (838) + 8 -> peek(SYSCALL_BASE + 838*8) -> peek(0x3B28F8 + 0x1A30)

#define LV2MOUNTADDR_480deh 0x80000000004D16C8ULL
//0xff0 => 0x116c (458098 - 459204)
#define LV2MOUNTADDR_480deh_ESIZE 0x118
#define LV2MOUNTADDR_480deh_CSIZE 0x108
...
static int lv2_unpatch_bdvdemu_480deh(void);
static int lv2_patch_bdvdemu_480deh(uint32_t flags);
static int lv2_patch_storage_480deh(void);
static int lv2_unpatch_storage_480deh(void);
...
int is_firm_480deh(void)
{
   ...
   {
      return (peekq(0x8000000000344B70ULL)==0x323031362F30342FULL); //timestamp: 2016/04
   }
   else
   {
      return 0;
...
int is_payload_loaded_480deh(void)
{
...
void set_bdvdemu_480deh(int current_payload)
{
    lv2_unpatch_bdvdemu = lv2_unpatch_bdvdemu_480deh;
    lv2_patch_bdvdemu   = lv2_patch_bdvdemu_480deh;
    lv2_patch_storage   = lv2_patch_storage_480deh;
    lv2_unpatch_storage = lv2_unpatch_storage_480deh;
}
...
*/
void load_payload_480deh(int mode)
{
/*
...
*/
    ...
    lv2_memcpy(0x8000000000000000ULL + (u64) PAYLOAD_OFFSET,
                   (u64) payload_sky_480deh_bin,
                   payload_sky_480deh_bin_size);

    lv2_memcpy(0x8000000000000000ULL + (u64) PAYLOAD_UMOUNT_OFFSET, // copy umount routine
                      (u64) umount_480deh_bin,
                      umount_480deh_bin_size);
    ...
    /*
			pokeq(0x800000000005A644ULL, 0x63FF003D60000000ULL ); // fix 8001003D error  Original: 0x63FF003D419EFFD4ULL
			pokeq(0x800000000005A404ULL, 0x3FE080013BE00000ULL ); // fix 8001003E error  Original: 0x3FE0800163FF003EULL

			pokeq(0x8000000000056600ULL, 0x419E00D860000000ULL ); // Original: 0x419E00D8419D00C0ULL
			pokeq(0x8000000000056608ULL, 0x2F84000448000098ULL ); // Original: 0x2F840004409C0048ULL //PATCH_JUMP
    ...
    */
			...
			pokeq(0x800000000022E710ULL, 0x38600000F8690000ULL ); // fix 0x8001002B / 80010017 errors
    ...
    _poke32(0x56604, 0x60000000);             //
    PATCH_JUMP(0x5660C, 0x566A4);             //
    _poke32(0x5E498,  0x60000000);            // fix 80010009 error
    _poke32(0x5E4AC,  0x60000000);            // fix 80010019 error
    _poke(  0x5A644,  0x63FF003D60000000); // fix 8001003D error  "ori     %r31, %r31, 0x3D\n nop\n" done
    _poke32(0x5A408,  0x3BE00000);            // fix 8001003E error -- 3.55 ok in 0x055F64 "li      %r31, 0"  done

    //Fix 0x8001003C error (incorrect version in sys_load_param) - It is present in the new game updates **/
    //_poke(26E52C, 0x386000007C6307B4); //
    //_poke32(26E52C + 0x8, 0x4E800020);  //
    ...
    */
    ...
    PATCH_JUMP(0x2C87F8, (PAYLOAD_OFFSET+0x30)); // patch openhook - done
...
static int lv2_patch_storage_480deh(void)
{
    ...
    pokeq32(0x800000000032C958ULL, 0x40000000); //FIXED
...
static int lv2_unpatch_storage_480deh(void)
{
    ...
    pokeq(0x800000000032C958ULL, save_lv2_storage_patch); //FIXED
...
static int lv2_unpatch_bdvdemu_480deh(void)
{
    ...
    sys8_memcpy((u64) mem, LV2MOUNTADDR_480deh, 0x10 * 0x118);
    sys8_memcpy((u64) (mem + 0x1200), 0x80000000007EF020ULL , LV2MOUNTADDR_480deh_CSIZE);

    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_480deh_ESIZE)
    {
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD1", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            if(!memcmp(mem + n + 0x69, "temp_bdvd", 10))
            {
                sys8_memcpy(LV2MOUNTADDR_480deh + n + 0x69, (u64) "dev_bdvd\0", 10);
                flag++;
            }
        }
        ...
        if(!memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            {
                sys8_memcpy(LV2MOUNTADDR_480deh + n + 0x69, (u64) (mem + n + 0x79), 11);
                sys8_memset(LV2MOUNTADDR_480deh + n + 0x79, 0ULL, 12);
                flag += 10;
            }
        }
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD0", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
              ...
              && !memcmp(mem + n + 0x79, "esp_bdvd", 9) && peekq(0x80000000007EF000ULL)!=0)
           {
                ...
                sys8_memcpy(LV2MOUNTADDR_480deh + (u64) (n - 0x10), (u64) (mem + 0x1200) , (u64) LV2MOUNTADDR_480deh_CSIZE);
                flag += 10;
           }
        }
    }
...
static int lv2_patch_bdvdemu_480deh(uint32_t flags)
{
    ...
    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_480deh_ESIZE)
    {
            ...
            && !memcmp(mem + n + 0x69, "dev_bdvd", 9) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            if(usb >= 0)
                sys8_memcpy(LV2MOUNTADDR_480deh + n + 0x69, (u64) "temp_bdvd", 10);
        ...
        {
            if(noBDVD) pos = -1;
            sys8_memcpy(LV2MOUNTADDR_480deh + n + 0x69, (u64) "dev_bdvd\0\0", 11);
            sys8_memcpy(LV2MOUNTADDR_480deh + n + 0x79, (u64) &path_name[128], 11);
    ...
    if(pos > 0 && pos2 > 0)
    {
      ...
      memcpy(mem + 0x1220, mem + pos2 - 0x10, LV2MOUNTADDR_480deh_CSIZE);
      dat = LV2MOUNTADDR_480deh + (u64) (pos2 - 0x10);
      ...
      n = (int) LV2MOUNTADDR_480deh_CSIZE; // CDATAS
      memcpy(mem + 0x1214, &n, 0x4);

      memcpy(mem + pos2, mem + pos, LV2MOUNTADDR_480deh_CSIZE - 0x10);
      memcpy(mem + pos2 + 0x69, "dev_bdvd\0\0", 11);
      memcpy(mem + pos2 + 0x79, "esp_bdvd\0\0", 11);
      memset(mem + pos2 + 0xa4, 0, 8);

      sys8_memcpy(0x80000000007EF000ULL , ((u64) mem + 0x1200), LV2MOUNTADDR_480deh_CSIZE + 0x20);
      sys8_memcpy(LV2MOUNTADDR_480deh + (u64) pos2, ((u64) (mem + pos2)), (u64) (LV2MOUNTADDR_480deh_CSIZE - 0x10));
