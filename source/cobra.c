/* cobre.c - Minimal Cobra library support for Game Launcher */
...
int cobra_set_psp_umd2(char *path, char *umd_root, char *icon_save_path, uint64_t options)
{
	...
	{
		DrawDialogOKTimer("Invalid ISO image path.", 2000.0f);
		return EINVAL;
	}
	...
	{
		DrawDialogOKTimer("Disc icon file not found.", 2000.0f);
		return -1;
	}
	...
	{
		DrawDialogOKTimer("PSP Launcher database not found.", 2000.0f);
		return -2;
	}
	...
	{
		DrawDialogOKTimer("Invalid ISO image.", 2000.0f);
		return EIO;
	}
	...
	if (!root)
	{
		...
		{
			DrawDialogOKTimer("Invalid disc type.", 2000.0f);
			return EBUSY;
		}
		...
		// Wait 0.5 seconds for auto mounter to mount ISO
		for (i = 0; i < 25; i++)
		{
			struct stat s;
		...
		if (fp)
		{
			...
			if (header[0] == 0x7E505350)
			{
				...
				if (i == NUM_SUPPORTED_TAGS)
				{
					...
					if (j == NUM_EXTRA_KEYS)
					{
						DrawDialogOKTimer("No tag file found. Game will crash.", 2000.0f);
					}
				}
				else
				{
