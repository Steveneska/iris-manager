	...
	if(ret == SUCCESS)
	{
		...
		if(ret == SUCCESS && pos == 0xCULL)
		{
			...
			if(ret == SUCCESS && bytesread == 4ULL && offset_fw > 0xF)
			{
				...
				{
					ret = sysLv2FsRead( file, &ver, 0x2, &bytesread ); // SELF/SPRX minimum firmware version
...
void copy_from_selection(int game_sel)
{
	...
	else if(fdevices & 1)
	{
		...
		if(dialog_action == 1)
		{
			...
			else if((directories[game_sel].flags & (PSP_FLAG)) == (PSP_FLAG))
			{
			...
			while(p[0] == '_') p++; // Skip special character
...
void copy_from_bluray()
{
	...
	for(n = 0; n < 11; n++)
	{
		...
		if((fdevices >> n) & 1)
		{
			...
			{
				if(n != 0)
					sprintf(progress_bar_title, "BDVD -> USB00%c", 47 + n);
				else
					sprintf(progress_bar_title, "BDVD -> HDD0");
					curr_device = n;
					break;
			}
		}
...
void test_game(int game_sel)
{
	...
	if(r)
		DPrintf("This game requires a higher CFW or rebuild the SELF/SPRX.");

	if(self_alarm_version)
		DPrintf("The update of this game requires a higher CFW or rebuild the SELF/SPRX.");
...
int getConfigMemValueString(char* mem, int size, char* pchSection, char* pchKey, char* pchValue, int iSize, char* pchDefaultValue)
{
	...
	while(n<size)
	{
		...
		if(find_section)
		{
			...
			// &chLine[m] contain the replacement string without string split ;)
