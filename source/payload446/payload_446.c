...
#include <ppu-lv2.h>
#include "payload.h"
#include "payload_446.h"
#include "syscall8.h"

#include "payload_sky_446_bin.h"
#include "umount_446_bin.h"
...
#define SYSCALL_BASE                    0x800000000035E860ULL // Done
#define NEW_POKE_SYSCALL                813
#define NEW_POKE_SYSCALL_ADDR           0x80000000001B5B9CULL // peek(SYSCALL_BASE + NEW_POKE_SYSCALL*8) -> peek(0x35E860 + 813*8)

#define PAYLOAD_OFFSET                  0x3d90
#define PERMS_OFFSET                    0x3560

#define PAYLOAD_UMOUNT_OFFSET           (0x3d90+0x400)
#define UMOUNT_SYSCALL_OFFSET           (0x1B42B4+0x8) // SYSCALL (838) + 8 -> peek(SYSCALL_BASE + 838*8) -> peek(0x35E860 + 0x1A30)

#define LV2MOUNTADDR_446 0x8000000000458098ULL // Done
//0xff0 => 0x116c (458098 - 459204)
#define LV2MOUNTADDR_446_ESIZE 0x118
#define LV2MOUNTADDR_446_CSIZE 0x108
...
static int lv2_patch_bdvdemu_446(uint32_t flags);
static int lv2_patch_storage_446(void);
static int lv2_unpatch_storage_446(void);
...
int is_firm_446(void)
{
    ...
    if(addr == NEW_POKE_SYSCALL_ADDR && peekq( 0x800000000000C448ULL ) == 0x614A60007F84E378ULL ) return 1;
...
int is_payload_loaded_446(void)
{
...
void set_bdvdemu_446(int current_payload)
{
    lv2_unpatch_bdvdemu = lv2_unpatch_bdvdemu_446;
    lv2_patch_bdvdemu   = lv2_patch_bdvdemu_446;
    lv2_patch_storage = lv2_patch_storage_446;
    lv2_unpatch_storage = lv2_unpatch_storage_446;
}
...
static int lv2_patch_storage_446(void)
{
    ...
    save_lv2_storage_patch= peekq(0x80000000002E9CC8ULL);
    pokeq32(0x80000000002E9CC8ULL, 0x40000000);
...
static int lv2_unpatch_storage_446(void)
{
    ...
    pokeq(0x80000000002E9CC8ULL, save_lv2_storage_patch);
...
static int lv2_patch_bdvdemu_446(uint32_t flags)
{
    ...
    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_446_ESIZE)
    {
            ...
            && !memcmp(mem + n + 0x69, "dev_bdvd", 9) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            if(usb >= 0)
                sys8_memcpy(LV2MOUNTADDR_446 + n + 0x69, (u64) "temp_bdvd", 10);
        ...
        {
            if(noBDVD) pos = -1;
            sys8_memcpy(LV2MOUNTADDR_446 + n + 0x69, (u64) "dev_bdvd\0\0", 11);
            sys8_memcpy(LV2MOUNTADDR_446 + n + 0x79, (u64) &path_name[128], 11);
    ...
    if(pos > 0 && pos2 > 0)
    {
      ...
      memcpy(mem + 0x1220, mem + pos2 - 0x10, LV2MOUNTADDR_446_CSIZE);
      dat = LV2MOUNTADDR_446 + (u64) (pos2 - 0x10);
      ...
      n = (int) LV2MOUNTADDR_446_CSIZE; // CDATAS
      memcpy(mem + 0x1214, &n, 0x4);

      memcpy(mem + pos2, mem + pos, LV2MOUNTADDR_446_CSIZE - 0x10);
      memcpy(mem + pos2 + 0x69, "dev_bdvd\0\0", 11);
      memcpy(mem + pos2 + 0x79, "esp_bdvd\0\0", 11);
      memset(mem + pos2 + 0xa4, 0, 8);

      sys8_memcpy(0x80000000007EF000ULL , ((u64) mem + 0x1200), LV2MOUNTADDR_446_CSIZE + 0x20);
      sys8_memcpy(LV2MOUNTADDR_446 + (u64) pos2, ((u64) (mem + pos2)), (u64) (LV2MOUNTADDR_446_CSIZE - 0x10));
