 ...
 *
 * "ControlFan" is a free software: you can redistribute it and/or modify
...
int get_controlfan_offsets()
{
	...
	{
		// Enable sys_game_get_temperature
		sys383_offset = 0x800000000000AEA0ULL;
		// Enable sys_sm_get_fan_policy
		sys409_offset = 0x8000000000008644ULL;
		// Enable sys_sm_set_fan_policy
	...
	else if((firmware == 0x480C) || (firmware == 0x490C)) // Firmware 4.80 CEX / 4.90 CEX // Firmware 4.80 CEX / 4.90 CEX
	{
		// Enable sys_game_get_temperature
		sys383_offset = 0x800000000000C6A4ULL;
		// Enable sys_sm_get_fan_policy
...
static int load_ps3_controlfan_sm_sprx()
{
	...
	if(lv2peek32(PAYLOAD_BASE) == 0x50534D58 && use_cobra)
	{
		...
		// Create sm.sprx if it does not exist
...
void set_device_wakeup_mode(u32 flags)
{
	...
	{
		for(int n = 1; n < 10; n++) {if(flags & (1<<n)) {flags = n - 1; mode = 1; break;}} // Test/use USB port
	}
