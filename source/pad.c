...
unsigned ps3pad_read()
{
	...
	if(count > 15)
	{
		...
		if(iTimeoutByInactivity)
		{
			...
			{
				if(DrawDialogYesNoTimer("The PS3™ system will be shutdown after two minutes of inactivity.\nDo you want to stop the countdown?", 120000.0f) != 1)
				{
					fun_exit();
					sys_shutdown();
					exit(0);
		...
		{
			DrawDialogOKTimer("CPU and RSX temperatures critical.\nThe PS3™ system will be shutdown in 10 seconds.\n", 10000.0f);
			fun_exit();
			sys_shutdown();
			exit(0);
		}
