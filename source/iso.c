...
static u32 dllba = 0;	// Directory entries
static u32 dwlba = 0;	// Directory entries
static u32 dlsz = 0;	// Directory entries size (sectors)
static u32 dwsz = 0;	// Directory entries size (sectors)
...
static int calc_entries(char *path, int parent)
{
	...
	// Calculate folders, sub-folders, multi-part files & files
	#ifdef NOPS3_UPDATE
	...
	if(dir)
	{
		...
		while(true)
		{
			...
			if(lname >= 6 && !strncmp(&entry->d_name[lname - 6], ".66600", 6))
			{
				...
				// Calculate multi-part file size
				for(int n = 1; n < 100; n++)
				{
					path[l] = 0;
			...
			for(int n = 0; n < parts; n++)
			{
				...
				add = sizeof(struct iso_directory_record) + lname - 1 + 8;
				add+= add & 1;
				cldir += add;
				...
				add = sizeof(struct iso_directory_record) + len_string * 2 + 9;
				add+= (add & 1);
				cwdir += add;
			...
			if(dir)
			{
				...
				while(true)
				{
					...
					if(is_dir_entry)
					{
						...
						add = sizeof(struct iso_directory_record) + lname + 5;
						add+= add & 1;
						cldir += add;
						...
						add = sizeof(struct iso_directory_record) + len_string * 2 + 5;
						add+= add & 1;
						cwdir += add;
...
static int fill_entries(char *path1, char *path2, int level)
{
	...
	if(dir)
	{
		...
		while(true)
		{
			...
			if(stat(path1, &s) != SUCCESS) continue;
			...
			for(n = 0; n < parts; n++)
			{
				...
				setdaterec(idrl->date, dd, mm, aa, ho, mi, se);
				idrl->flags[0] = ((n + 1) != parts) ? 0x80 : 0x0;
				idrl->file_unit_size[0] = 0x0;
				idrl->interleave[0] = 0x0;
				set723(idrl->volume_sequence_number, 1);
				...
				setdaterec(idrw->date, dd, mm, aa, ho, mi, se);
				idrw->flags[0] = ((n + 1) != parts) ? 0x80 : 0x0;
				idrw->file_unit_size[0] = 0x0;
				idrw->interleave[0] = 0x0;
				set723(idrw->volume_sequence_number, 1);
	...
	for(n = 1; n < cur_isop; n++)
		if(directory_iso[n].parent == level + 1)
		{
			...
			add = sizeof(struct iso_directory_record) + len_string + 5;
			add+= (add & 1);
...
int makeps3iso(char *g_path, char *f_iso, int split)
{
	...
	// Create path to get free space from destination file
	...
	if(nlen < 1) {strcpy(output_name, path2);}
	else
	{
	...
	dllba = 0;	// Directory entries
	dwlba = 0;	// Directory entries
	dlsz = 0;	// Directory entries size (sectors)
	dwsz = 0;	// Directory entries size (sectors)
	...
	set733((void *) &idr->size[0], directory_iso[0].ldir * SECTOR_SIZE);
	...
	set733((void *) &idr->size[0], directory_iso[0].wdir * SECTOR_SIZE);
	...
	if(fd2 >= 0)
	{
		...
		if(ret != ERROR_CREATING_SPLIT_FILE)
		{
...
static int iso_patch_exe_error_09(u32 lba, char *filename)
{
	...
	// Open SELF/SPRX and changes the firmware version
	...
	if(firmware >= 0x421C && firmware < 0x485C && ver > fw_421 && ver <= fw_488 && ver > cur_firm)
	{
...
int patchps3iso(char *f_iso, int nopause)
{
	...
	u32 lba0 = isonum_731(&sect_descriptor.type_l_path_table[0]); // LBA
	u32 size0 = isonum_733(&sect_descriptor.path_table_size[0]);
