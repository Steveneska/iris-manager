...
#include <ppu-lv2.h>
#include "payload.h"
#include "payload_470.h"
#include "syscall8.h"

#include "payload_sky_470_bin.h"
#include "umount_470_bin.h"
...
#define LV2MOUNTADDR_470_CSIZE 0x108
...
static int lv2_unpatch_bdvdemu_470(void);
static int lv2_patch_bdvdemu_470(uint32_t flags);
static int lv2_patch_storage_470(void);
static int lv2_unpatch_storage_470(void);
...
int is_payload_loaded_470(void)
{
...
void set_bdvdemu_470(int current_payload)
{
    lv2_unpatch_bdvdemu = lv2_unpatch_bdvdemu_470;
    lv2_patch_bdvdemu   = lv2_patch_bdvdemu_470;
    lv2_patch_storage = lv2_patch_storage_470;
    lv2_unpatch_storage = lv2_unpatch_storage_470;
}
...
static inline void remove_lv2_memcpy()
{
    ...
    /* restore syscall */
    //remove_new_poke();
// Found @ 0x2A1408 This is per FW, syscall_rmdir_desc // SC813
        pokeq(NEW_POKE_SYSCALL_ADDR + 0x00, 0xF821FF017C0802A6ULL);
        pokeq(NEW_POKE_SYSCALL_ADDR + 0x08, 0xFBC100F0FBE100F8ULL);
        pokeq(NEW_POKE_SYSCALL_ADDR + 0x10, 0xEBC225B07C7F1B78ULL);
...
*/
    ...
    _poke(0x2670DC, 0x386000007C6307B4); //
    _poke32(0x2670DC + 8, 0x4E800020);  //
...
static int lv2_patch_storage_470(void)
{
    ...
    save_lv2_storage_patch= peekq(0x80000000002ECA30ULL);
    pokeq32(0x80000000002ECA30ULL, 0x40000000); //FIXED
...
static int lv2_unpatch_storage_470(void)
{
    ...
    pokeq(0x80000000002ECA30ULL, save_lv2_storage_patch); //FIXED
...
static int lv2_unpatch_bdvdemu_470(void)
{
    ...
    sys8_memcpy((u64) mem, LV2MOUNTADDR_470, 0x10 * 0x118);
    sys8_memcpy((u64) (mem + 0x1200), 0x80000000007EF020ULL , LV2MOUNTADDR_470_CSIZE);

    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_470_ESIZE)
    {
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD1", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            if(!memcmp(mem + n + 0x69, "temp_bdvd", 10))
            {
                sys8_memcpy(LV2MOUNTADDR_470 + n + 0x69, (u64) "dev_bdvd\0", 10);
                flag++;
            }
        }
        ...
        if(!memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            {
                sys8_memcpy(LV2MOUNTADDR_470 + n + 0x69, (u64) (mem + n + 0x79), 11);
                sys8_memset(LV2MOUNTADDR_470 + n + 0x79, 0ULL, 12);
                flag += 10;
            }
        }
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD0", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
              ...
              && !memcmp(mem + n + 0x79, "esp_bdvd", 9) && peekq(0x80000000007EF000ULL)!=0)
           {
                ...
                sys8_memcpy(LV2MOUNTADDR_470 + (u64) (n - 0x10), (u64) (mem + 0x1200) , (u64) LV2MOUNTADDR_470_CSIZE);
                flag += 10;
           }
        }
    }
...
static int lv2_patch_bdvdemu_470(uint32_t flags)
{
    ...
    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_470_ESIZE)
    {
            ...
            && !memcmp(mem + n + 0x69, "dev_bdvd", 9) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            if(usb >= 0)
                sys8_memcpy(LV2MOUNTADDR_470 + n + 0x69, (u64) "temp_bdvd", 10);
        ...
        {
            if(noBDVD) pos = -1;
            sys8_memcpy(LV2MOUNTADDR_470 + n + 0x69, (u64) "dev_bdvd\0\0", 11);
            sys8_memcpy(LV2MOUNTADDR_470 + n + 0x79, (u64) &path_name[128], 11);
    ...
    if(pos > 0 && pos2 > 0)
    {
      ...
      memcpy(mem + 0x1220, mem + pos2 - 0x10, LV2MOUNTADDR_470_CSIZE);
      dat = LV2MOUNTADDR_470 + (u64) (pos2 - 0x10);
      ...
      n = (int) LV2MOUNTADDR_470_CSIZE; // CDATAS
      memcpy(mem + 0x1214, &n, 0x4);

      memcpy(mem + pos2, mem + pos, LV2MOUNTADDR_470_CSIZE - 0x10);
      memcpy(mem + pos2 + 0x69, "dev_bdvd\0\0", 11);
      memcpy(mem + pos2 + 0x79, "esp_bdvd\0\0", 11);
      memset(mem + pos2 + 0xa4, 0, 8);

      sys8_memcpy(0x80000000007EF000ULL , ((u64) mem + 0x1200), LV2MOUNTADDR_470_CSIZE + 0x20);
      sys8_memcpy(LV2MOUNTADDR_470 + (u64) pos2, ((u64) (mem + pos2)), (u64) (LV2MOUNTADDR_470_CSIZE - 0x10));
