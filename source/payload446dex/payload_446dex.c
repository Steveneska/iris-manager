...
#include <ppu-lv2.h>
#include "payload.h"
#include "payload_446dex.h"
#include "syscall8.h"

#include "payload_sky_446dex_bin.h"
#include "umount_446dex_bin.h"
...
#define SYSCALL_BASE                    0x800000000037CFE8ULL
#define NEW_POKE_SYSCALL                813
#define NEW_POKE_SYSCALL_ADDR           0x80000000001BBF8CULL   // where above syscall is in lv2
#define PAYLOAD_OFFSET                  0x3d90
#define PERMS_OFFSET                    0x3560

#define PAYLOAD_UMOUNT_OFFSET           (0x3d90+0x400)
#define UMOUNT_SYSCALL_OFFSET           (0x1BA6A4+0x8) // SYSCALL (838)
#define LV2MOUNTADDR_446dex 0x8000000000478098ULL
//0xff0 => 0x116c (458098 - 459204)
#define LV2MOUNTADDR_446dex_ESIZE 0x118
#define LV2MOUNTADDR_446dex_CSIZE 0x108
...
static int lv2_unpatch_bdvdemu_446dex(void);
static int lv2_patch_bdvdemu_446dex(uint32_t flags);
static int lv2_patch_storage_446dex(void);
static int lv2_unpatch_storage_446dex(void);
...
int is_firm_446dex(void)
{
    ...
    if(addr == NEW_POKE_SYSCALL_ADDR && peekq( 0x800000000000C4C8ULL ) == 0x614A60007F84E378ULL ) return 1;
...
int is_payload_loaded_446dex(void)
{
...
void set_bdvdemu_446dex(int current_payload)
{
    lv2_unpatch_bdvdemu = lv2_unpatch_bdvdemu_446dex;
    lv2_patch_bdvdemu   = lv2_patch_bdvdemu_446dex;
    lv2_patch_storage = lv2_patch_storage_446dex;
    lv2_unpatch_storage = lv2_unpatch_storage_446dex;
}
...
static inline void remove_lv2_memcpy()
{
    ...
    /* restore syscall */
    //remove_new_poke();
// Found @ 0x1BBF8C This is per FW, syscall_rmdir_desc
...
static int lv2_patch_storage_446dex(void)
{
    ...
    save_lv2_storage_patch= peekq(0x8000000000304720ULL);
    pokeq32(0x8000000000304720ULL, 0x40000000);
...
static int lv2_unpatch_storage_446dex(void)
{
    ...
    pokeq(0x8000000000304720ULL, save_lv2_storage_patch);
...
static int lv2_unpatch_bdvdemu_446dex(void)
{
    ...
    sys8_memcpy((u64) mem, LV2MOUNTADDR_446dex, 0x10 * 0x118);
    sys8_memcpy((u64) (mem + 0x1200), 0x80000000007EF020ULL , LV2MOUNTADDR_446dex_CSIZE);

    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_446dex_ESIZE)
    {
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD1", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            if(!memcmp(mem + n + 0x69, "temp_bdvd", 10))
            {
                sys8_memcpy(LV2MOUNTADDR_446dex + n + 0x69, (u64) "dev_bdvd\0", 10);
                flag++;
            }
        }
        ...
        if(!memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            {
                sys8_memcpy(LV2MOUNTADDR_446dex + n + 0x69, (u64) (mem + n + 0x79), 11);
                sys8_memset(LV2MOUNTADDR_446dex + n + 0x79, 0ULL, 12);
                flag += 10;
            }
        }
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD0", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
              ...
              && !memcmp(mem + n + 0x79, "esp_bdvd", 9) && peekq(0x80000000007EF000ULL)!=0)
           {
                ...
                sys8_memcpy(LV2MOUNTADDR_446dex + (u64) (n - 0x10), (u64) (mem + 0x1200) , (u64) LV2MOUNTADDR_446dex_CSIZE);
                flag += 10;
           }
        }
    }
...
static int lv2_patch_bdvdemu_446dex(uint32_t flags)
{
    ...
    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_446dex_ESIZE)
    {
            ...
            && !memcmp(mem + n + 0x69, "dev_bdvd", 9) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            if(usb >= 0)
                sys8_memcpy(LV2MOUNTADDR_446dex + n + 0x69, (u64) "temp_bdvd", 10);
        ...
        {
            if(noBDVD) pos = -1;
            sys8_memcpy(LV2MOUNTADDR_446dex + n + 0x69, (u64) "dev_bdvd\0\0", 11);
            sys8_memcpy(LV2MOUNTADDR_446dex + n + 0x79, (u64) &path_name[128], 11);
    ...
    if(pos > 0 && pos2 > 0)
    {
      ...
      memcpy(mem + 0x1220, mem + pos2 - 0x10, LV2MOUNTADDR_446dex_CSIZE);
      dat = LV2MOUNTADDR_446dex + (u64) (pos2 - 0x10);
      ...
      n = (int) LV2MOUNTADDR_446dex_CSIZE; // CDATAS
      memcpy(mem + 0x1214, &n, 0x4);

      memcpy(mem + pos2, mem + pos, LV2MOUNTADDR_446dex_CSIZE - 0x10);
      memcpy(mem + pos2 + 0x69, "dev_bdvd\0\0", 11);
      memcpy(mem + pos2 + 0x79, "esp_bdvd\0\0", 11);
      memset(mem + pos2 + 0xa4, 0, 8);

      sys8_memcpy(0x80000000007EF000ULL , ((u64) mem + 0x1200), LV2MOUNTADDR_446dex_CSIZE + 0x20);
      sys8_memcpy(LV2MOUNTADDR_446dex + (u64) pos2, ((u64) (mem + pos2)), (u64) (LV2MOUNTADDR_446dex_CSIZE - 0x10));
